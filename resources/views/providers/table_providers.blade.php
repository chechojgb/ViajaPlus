<x-app-layout>
    ...
    <x-empty-form  title="Providers" message="Your search “Stripe” did not match any Providers. Please try again or create a new Provider."  actionUrl="{{ route('providers.create') }}" buttonText="Add Provider" />
                                            ...
                                            <tr>
                                                <td class="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                    <div class="inline-flex items-center gap-x-3">
                                                        ...
                                                        <div class="flex items-center gap-x-2">
                                                            ...
                                                            <div>
                                                                <h2 class="font-medium text-gray-800 dark:text-white ">{{$provider->name}}</h2>
                                                                <p class="text-sm font-normal text-gray-600 dark:text-gray-400">@.{{$provider->name}}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
                                                    <div class="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                                                        ...
                                                        <h2 class="text-sm font-normal text-emerald-500">{{$provider->status}}</h2>
                                                    </div>
                                                </td>
                                                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{$provider->address}}</td>
                                                <td class="px-4 py-4 text-sm text-gray-500 dark:text-gray-300 whitespace-nowrap">{{$provider->email}}</td>
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div class="flex items-center gap-x-2">
                                                        <p class="px-3 py-1 text-xs text-indigo-500 rounded-full dark:bg-gray-800 bg-indigo-100/60">{{$provider->provider_type}}</p>
                                                    </div>
                                                </td>
                                                <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                    <div class="flex items-center gap-x-6">

                                                        <div x-data="{ isOpen: false, idForm: null, type: '', message: '' }" >

                                                            <button @click="isOpen = true; idForm = '{{ route('providers.delete', ['id' => $provider->id]) }}'; type = '{{ __('Providers') }}'; message = '{{ __('Are you sure you want to delete this resource?') }}'" 
